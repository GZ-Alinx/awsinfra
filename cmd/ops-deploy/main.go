...
func run() error {
	...
	initDatastore := flags.String("datastore", "", "init datastore mode: local or external; empty prompts interactively")
	initSkipDatastoreCheck := flags.Bool("skip-datastore-check", false, "write external datastore settings without testing connectivity")
	...
	if action == "init" {
		return initEnvironmentCLI(*configPath, *initOutput, *initDatastore, *initSkipDatastoreCheck)
	}
	// Version inspection must work directly from a downloaded archive, before
	// the operator has created .env or connected MySQL/Redis.
	if action == "version" {
